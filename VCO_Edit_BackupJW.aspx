...
<asp:Content ID="Content3" ContentPlaceHolderID="CPH_Form" runat="server">
    ...
    <script type="text/javascript">
        ...
        function drop(ev) {
            ...
            if (t[0] == "Freitext") {
                var p = data.indexOf(" ");
                var ftxt = data.substr(p + 1);
                var sz = document.getElementById("selSize");
                var ff = document.getElementById("selFontfamily");
                var ft = sz.value + ff.value;
                var col = document.getElementById("selColor").value;
                var bgcol = document.getElementById("selBgCol").value;
                var ssz = sz.value;
                var sn = parseInt(ssz.substring(0, ssz.indexOf("px")));
                var FreitextObject = new Object();
                FreitextObject["Freitext"] = ftxt;
                FreitextObject["bmpIndex"] = bmpIndex;
                FreitextObject["x"] = CurrentX;
                FreitextObject["y"] = CurrentY;
                // Usrsprüngliche Positionen bei Eingabe merken
                FreitextObject["_x"] = CurrentX;
                FreitextObject["_y"] = CurrentY;
                FreitextObject["font"] = ft;
                FreitextObject["Color"] = col;
                if (bgcol == "")
                    bgcol = $(".vinsideWrapper").css("background-color");
                FreitextObject["BgColor"] = bgcol;
                FreitextObject["BgHeight"] = sn;

                // Freitext als Verweis verwenden
                FreitextObject["isVerweis"] = TextIsVerweis;
                FreitextObject["VerweisAusrichtung"] = VerweisAusrichtung;
                FreitextObject["idxVerweisBitmap"] = idxBitmap;


                FreitextList.push(FreitextObject);
                ...
                DrawObject["bmpIndex"] = bmpIndex;
                DrawObject["VCOItem"] = CurrentVCOItem;
                DrawObject["x"] = CurrentX;
                DrawObject["y"] = CurrentY;
                DrawObject["font"] = ft;
                DrawObject["Color"] = col;
                DrawObject["ToolTip"] = CurrentVCOItem.Kommentar;

                DrawObject["Symbol"] = "";
                DrawObject["SymbolFeature"] = "";

                // ggf. Symbol Initialisierung fordern
                if (CurrentVCOItem.isBool)
                    DrawObject["ShowSymbolMenue"] = true;

                if (bgcol == "")
                    bgcol = $(".vinsideWrapper").css("background-color");
                DrawObject["BgColor"] = bgcol;
                DrawObject["BgHeight"] = sn;
                DropList.push(DrawObject);
        ...
        function drawVCOItem(item) {
            if (item["bmpIndex"] == bmpIndex) {
                ...
                var nk = item.NKStellen;
                var x = item.x;
                var y = item.y;
                var vco = item["VCOItem"];
                var txt = vco.Bez + " " + vco.Kanal.toString() + " " + vco.sEinheit;
        ...
        function HandleSymbolFeatures(value) {
            ...
            if (value == "HKTooltip") {
                $("#SelectSymbolFeature").css("display", "none");
            }
            if (value == "Heizkreis") {
                $("#SelectSymbolFeature").css("display", "none");
            }
        ...
        // Symbole zeichnen
        function drawSymbolAndFeature(item) {
            var sym = selectedSymbol;
            var feat; // = selectedSymbolFeature;
            var itm = item;
            // bei Neuanlage in Droplist noch nicht gesetzt, aber im Auswahlmenue
            if (itm == null) {
                itm = CurrentDroplistItem;

            }
            if (itm.ShowSymbolMenue == true) {
                // Neuer Eintrag. in Droplist setzen
                itm.ShowSymbolMenue = false;
                itm.Symbol = sym;
                var select = document.getElementById('SelectSymbolFeature');
                feat = select.value;
                itm.SymbolFeature = feat;
            }
            // nun zeichnen

            if (itm.Symbol == "Heizkreis") {
                Heizkreis(ctx, itm.x, itm.y, 1);
            }

            if (itm.Symbol == "Absenkung") {
                Absenkung(ctx, itm.x, itm.y, 1, 0);
            }

            if (itm.Symbol == "HKTooltip") {
                hktooltip(ctx, itm.x, itm.y, 1);
            }

            if (itm.Symbol == "BHKW") {
                BHDreh(ctx, itm.x, itm.y, 1, 0);
            }
            if (itm.Symbol == "Feuer") {
                feuer(ctx, itm.x, itm.y, 1);
            }
            if (itm.Symbol == "Pumpe") {
                pmpDreh2(ctx, itm.x, itm.y, 1, 0);
            }
            if (itm.Symbol == "Luefter") {
                if (itm.SymbolFeature == "Links") {
                    luefter(ctx, itm.x, itm.y, 1, 30, 0);
                }
                if (itm.SymbolFeature == "Rechts") {
                    luefter(ctx, itm.x, itm.y, 1, 30, 180);
                }
                if (itm.SymbolFeature == "Oben") {
                    luefter(ctx, itm.x, itm.y, 1, 30, 90);
                }
                if (itm.SymbolFeature == "Unten") {
                    luefter(ctx, itm.x, itm.y, 1, 30, 270);
                }
            }
            if (itm.Symbol == "Ventil") {
                if (itm.SymbolFeature == "Links") {
                    ventil(ctx, itm.x, itm.y, 2, 180);
                }
                if (itm.SymbolFeature == "Rechts") {
                    ventil(ctx, itm.x, itm.y, 2, 0);
                }
                if (itm.SymbolFeature == "Oben") {
                    ventil(ctx, itm.x, itm.y, 2, 270);
                }
                if (itm.SymbolFeature == "Unten") {
                    ventil(ctx, itm.x, itm.y, 2, 90);
                }
            }

            if (itm.Symbol == "Abluftklappen") {
                ablufklappen(ctx, itm.x, itm.y, 1, "auf");
            }

            if (itm.Symbol == "Led") {
                if (itm.SymbolFeature == "unsichtbar/rot") {
                    Led(ctx, itm.x, itm.y, 1, "red");
                }
                if (itm.SymbolFeature == "gruen/rot") {
                    Led(ctx, itm.x, itm.y, 1, "green");
                }
                if (itm.SymbolFeature == "rot/gruen") {
                    Led(ctx, itm.x, itm.y, 1, "green");
                }
                if (itm.SymbolFeature == "unsichtbar/rot blinkend") {
                    Led(ctx, itm.x, itm.y, 1, "magenta");
                }
                if (itm.SymbolFeature == "gruen/rot blinkend") {
                    Led(ctx, itm.x, itm.y, 1, "#f0f");
                }
                if (itm.SymbolFeature == "gruen/unsichtbar") {
                    Led(ctx, itm.x, itm.y, 1, "green");
                }
            }
            // Menue verlassen
        ...
        // aktuellen Stand als Deploy rpi
        function uploadVisuRTOS(vto) {
            var res;
            $.ajax({
                type: "POST",
                url: "WebServiceEK.asmx/DeployVisuRTOS",
                data: vto,
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                async: false,
                    ...
                    <div id="Div1" class="imgSelect">
                       ...
                       <select id="selSize">
                        ...
                        <select id="selBgCol">
                            ...
                            <option value="#E0E0E0" selected="selected">zaehlerbg</option>
        ...
        <div class="row">
            <div class="col-md-12">
                <div id="Buttonbar" class="Buttonbar">
                    ...
                    <input id="Button10" type="button" value="Upload to RTOS" class="cmdButton" onclick="VisuToRTOS()" />
                </div>
            </div>
        </div>
    ...
    <div id="EditSymbol" class="EditSymbol">
        <div id="eftDiv">
            ...
            Absenkung<br />
            <input id="Radio9" name="rgSymbols" type="radio" value="Abluftklappen" onclick="HandleSymbolFeatures(value)" />
            Abluftklappen<br />
             <input id="Radio10" name="rgSymbols" type="radio" value="Heizkreis" onclick="HandleSymbolFeatures(value)" />
            Heizkreis<br />
            <br />
